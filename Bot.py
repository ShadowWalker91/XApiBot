...
def set_twitter_auth(credentials):
    ...
    #auth = ty.OAuth2BearerHandler(BEARER_TOKEN)
    api = ty.API(auth ,wait_on_rate_limit=True, wait_on_rate_limit_notify=True, retry_count=10, retry_delay=900, retry_errors=5)
...
def get_user_followers(api, screen_name):
    ids = []
    counting = 0
    for page in ty.Cursor(api.followers_ids, screen_name=screen_name).pages():
        print('Hellofkofsdofsd')
        counting += 1
        ids.extend(page)
        print('sadsdfsdfgbghfhnhghb')
        time.sleep(1)
        print(counting)
        if counting == 5:
            counting = 0
...
def user_follow(api,user_id):
        try:
            user_id = str(user_id)
            out_put = api.create_friendship(user_id=user_id)
            print(print(out_put))
...
def fetch_hashtag_tweets(account_array, hashtag): #
    ...
    for account in account_array:
        ...
        hashtag_tweets = api.search(hashtag,'','','','','','','','recent',100)
        count = 0
...
def Unfollow_specific_acc_from_all_acc(account_array, screen_name_to_unfollow):
    ...
    for account in account_array:
        api = set_twitter_auth(account)
        follow_user = api.get_user(screen_name_to_unfollow)
        ...
        if check_whitelist_id(follow_user.id):
            ...
            print("Unfollowed", screen_name_to_unfollow, "from: " , api.me().screen_name)
    ...
    for account in account_array:
        ...
        for friend in following_ids:
            if friend in followers_ids:
                if check_whitelist(screen_name) | check_whitelist_id(user_id):
                    print("User In whitelist. Cannot Unfollow" , screen_name)
                else:
                    unfollow_user(api, id)
                    custom_delay(180)
    ...
    for account in account_array:
        ...
        for friend in following_ids:
            if count < 5000:
                ...
                custom_delay(3)
                count = count + 1
                print ("Unfollowing: ",api.get_user(friend).screen_name, " using Bot\n", count, " users unfollowed")
...
def unfollow_non_follow_backs(account_array):
    ...
    for account in account_array:
        ...
        for non_follower in non_followers:
            if count <= 5000:
                if check_whitelist_id(non_follower):
                        print("User In whitelist. Cannot Unfollow" , api.get_user(non_follower).screen_name)
                else:
                    unfollow_user(api, non_follower)
                    custom_delay(3)
                    count = count + 1
                    print("unfollowed", api.get_user(non_follower).screen_name)
...
def clear_favorites(account_array):
    ...
    for account in account_array:
        ...
        #Geting Favorites
        favourites = []
        favorite_ids = api.favorites(screen_name, [1,10])
...
def unfollow_actions(account_array,selection):
    ...
    if int(selection) == 1:
        ...
        for account in accounts:
            ...
            followers_ids = get_user_followers(api, screen_name)
            for follower_id in followers_ids:
                state = dp_check(api.get_user(follower_id))
                if state == True:
        ...
        for account in accounts:
            ...
            followers_ids = get_user_followers(api, screen_name)
            for follower_id in followers_ids:
                state = last_tweeted(api.get_user(follower_id))
                if state == False:
                    unfollow_user(api, follower_id)
    elif int(selection) == 4:
        for account in accounts:
    ...
    for account in account_array:
        ...
        for user_following_id in user_following_ids:
            ...
            if user_following_id not in following_ids:
                if check_blacklist(api.get_user(user_following_id)) | check_blacklist_id(user_id):
                    print(api.get_user(user_following_id), " is in blacklist. Cannot Follow")
                else:
                    user_follow(api, user_following_id)
                    count = count + 1
                    print("Followed: ", api.get_user(user_following_id).screen_name, " from account: ", screen_name)
        print("Followed ", count, "users from account: ", screen_name)
...
def follow_followers_of_following_of_someone(account_array, user_name_for_followers):
    ...
    for account in account_array:
        ...
        count = 0
        for user_following_id in user_following_ids:
           level_2_follower_ids = get_user_followers(api, api.get_user(user_following_id).screen_name)
           for level_2_follower_id in level_2_follower_ids:
               if level_2_follower_id not in following_ids:
                   if check_blacklist(api.get_user(level_2_follower_id).screen_name) | check_blacklist_id(level_2_follower_id):
                       print(api.get_user(level_2_follower_id).scree_name, " is in blacklist. Cannot Follow")
                   else:
                       user_follow(api, level_2_follower_id)
                       count = count + 1
                       print("Followed: ", api.get_user(level_2_follower_id).screen_name, " from account: ", screen_name)
           print("Followed ", count, "users from account: ", screen_name)
...
def follow_following_of_following_of_someone(account_array, user_name_for_followers):
    ...
    for account in account_array:
        ...
        count = 0
        for user_following_id in user_following_ids:
            level_2_following_ids = get_user_friends(api, api.get_user(user_following_id).screen_name)
            for level_2_following_id in level_2_following_ids:
                if level_2_following_id not in following_ids:
                    if check_blacklist(api.get_user(level_2_following_id).screen_name) | check_blacklist_id(level_2_following_id):
                        print(api.get_user(level_2_following_id).scree_name, " is in blacklist. Cannot Follow")
                    else:
                        user_follow(api, level_2_following_id)
                        count = count + 1
                        print("Followed: ", api.get_user(level_2_following_id).screen_name, " from account: ", screen_name)
            print("Followed ", count, "users from account: ", screen_name)
